...
CREATE OR ALTER VIEW GDDIENTOS.VW_Ganancias
AS
	SELECT
...
CREATE OR ALTER VIEW GDDIENTOS.VW_Factura_Promedio
AS
	SELECT
...
CREATE OR ALTER VIEW GDDIENTOS.VW_Rendimiento_Modelos
AS
	SELECT
		t.bi_tiempo_anio as [Anio],
...
CREATE OR ALTER VIEW GDDIENTOS.VW_Volumen_Pedidos
AS
	SELECT
...
CREATE OR ALTER VIEW GDDIENTOS.VW_Conversion_Pedidos
AS
	SELECT
...
CREATE OR ALTER VIEW GDDIENTOS.VW_Tiempo_Promedio_Fabricacion
AS
	SELECT
		bi_tiempo_anio as [Anio],
		bi_tiempo_cuatrimestre as [Cuatrimestre],
...
CREATE OR ALTER VIEW GDDIENTOS.VW_Promedio_Compras
AS
...
CREATE OR ALTER VIEW GDDIENTOS.VW_Compras_Por_Tipo_Material
AS
	SELECT
...
CREATE OR ALTER VIEW GDDIENTOS.VW_Porcentaje_Cumplimiento_Envios
AS
	SELECT
		bi_tiempo_anio AS Anio,
		bi_tiempo_mes AS Mes,
...
CREATE OR ALTER VIEW GDDIENTOS.VW_Localidades_Con_Mayor_Costo_De_Envio
AS
	SELECT
	...
	SELECT TOP 3
		e2.bi_envios_ubicacion_cliente
